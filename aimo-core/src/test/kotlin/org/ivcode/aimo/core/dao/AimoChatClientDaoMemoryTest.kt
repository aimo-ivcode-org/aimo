...
class AimoChatClientDaoMemoryTest {
    ...
    fun `getChatRequests with maxRequestCharacters returns newest requests within budget`() {
        ...
        dao.addChatRequest("user1", request(chatId, 1, 10, "r1"))
        dao.addChatRequest("user1", request(chatId, 2, 20, "r2"))
        dao.addChatRequest("user1", request(chatId, 3, 30, "r3"))
    ...
    fun `getChatRequests with maxRequestCharacters returns empty when newest exceeds budget`() {
        ...
        dao.addChatRequest("user1", request(chatId, 1, 10, "r1"))
        dao.addChatRequest("user1", request(chatId, 2, 40, "r2"))
    ...
    fun `getChatRequests with maxRequestCharacters preserves complete requests`() {
        ...
        val result = dao.getChatRequests("user1", chatId, maxRequestCharacters = 8)
    ...
    fun `getChatRequests with budget 0 returns empty list`() {
        ...
        repeat(3) { i ->
            dao.addChatRequest("user1", request(chatId, i + 1, 10, "msg-$i"))
        }
    ...
    fun `getChatRequests with budget less than one message returns empty`() {
        ...
        assertEquals(emptyList(), dao.getChatRequests("user1", chatId, maxRequestCharacters = 5))
    }
    ...
    fun `getChatRequests with budget equal to one message returns exactly one message`() {
        ...
        val result = dao.getChatRequests("user1", chatId, maxRequestCharacters = 10)
    ...
    fun `getChatRequests with budget exceeding two messages still returns only two`() {
        ...
        val result = dao.getChatRequests("user1", chatId, maxRequestCharacters = 25)
        assertEquals(listOf("msg-1", "msg-2"), result.map { it.messages.single().content })
    }
    ...
    fun `getChatRequests respects chronological order`() {
        ...
        dao.addChatRequest("user1", request(chatId, 1, 10, "first"))
        dao.addChatRequest("user1", request(chatId, 2, 20, "second"))
        dao.addChatRequest("user1", request(chatId, 3, 30, "third"))
        ...
        val result2 = dao.getChatRequests("user1", chatId, maxRequestCharacters = 60)
    ...
    fun `getChatRequests with variable message sizes`() {
        ...
        dao.addChatRequest("user1", request(chatId, 1, 5, "small"))
        dao.addChatRequest("user1", request(chatId, 2, 15, "medium"))
        dao.addChatRequest("user1", request(chatId, 3, 8, "tiny"))
        ...
        val result2 = dao.getChatRequests("user1", chatId, maxRequestCharacters = 23)
